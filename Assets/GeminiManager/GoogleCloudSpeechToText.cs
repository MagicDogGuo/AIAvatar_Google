...
namespace GoogleSpeechToText.Scripts
{
    public class GoogleCloudSpeechToText : MonoBehaviour
    {
        ...
        public static void SendSpeechToTextRequest(byte[] bytes, string apiKey, int sampleRateHertz, int audioChannelCount, string languageCode, Action<string> onSuccess, Action<BadRequestData> onError)
        {
            ...
            var requestData = new SpeechToTextRequest
            {
                ...
                {
                    encoding = "LINEAR16",
                    // 這裡一定要填正確，否則可能會辨識不到或品質很差（特別是送 WAV/LINEAR16 時）
                    sampleRateHertz = sampleRateHertz,
                    audioChannelCount = audioChannelCount,
                    // 預設改成可從外部帶入（例如 zh-TW / zh-CN / en-US）
        ...
        private static bool HasError(UnityWebRequest request, out BadRequestData badRequestData)
        {
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(responseText))
                    throw new Exception("Empty error response body");
            ...
            catch (Exception)
            {
                badRequestData = new BadRequestData
                {
                    error = new Error
                    {
                        ...
                        message = !string.IsNullOrWhiteSpace(responseText) ? responseText : request.error
                    }
                };
                return true;
            }
